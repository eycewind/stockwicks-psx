...
def determine_signals(df: pd.DataFrame) -> pd.DataFrame:
    """
    Generate more responsive buy/sell signals based on SuperTrend
    """
    df = df.copy()
    ...
    # Remove consecutive same signals (no cooldown between opposite signals)
    return df

# --- Enhanced Backtest Core ---
    ...
    for i in range(1, len(df)):
        ...
        if position is not None and stop_loss_price is not None:
            if position == "long" and current_low <= stop_loss_price:
                exit_price = max(stop_loss_price, current_low)  # Use worse price for stop loss
                ...
                exit_price = min(stop_loss_price, current_high)  # Use worse price for stop loss
...
def run(symbol: str, interval: str, trade_size: float, user_id: str, eod_close: bool, fixed_stop_loss_amount: float):
