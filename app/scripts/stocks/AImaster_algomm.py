#!/usr/bin/env python3
# /var/www/stockwicks/app/scripts/stocks/AImaster_algomm.py
...
# Only AlgoMM for this master
ALGO_SCRIPTS = {
    "AlgoMM": os.path.join(REPO_ROOT, "app/scripts/stocks/backtest_algos/AlgoMM_eval.py"),
}

# Intervals to test for each symbol
# If Schwab errors are annoying for some, you can comment out specific ones.
INTERVALS_TO_TEST = ["1min", "5min", "10min", "15min", "30min"]
...
def run_single_backtest(
    ...
    and Algo-specific flags, then reads the per-interval summary to compute
    Total_Profit and Success_Rate.
    """
    try:
        ...
        if algo_name == "AlgoMM":
            # Auto-train if model missing, use 3-bar horizon + averaging
            cmd.extend([
                "--auto-train",
                "--k-forward", "3",
                # You can also hardcode defaults if you want:
                # "--builder-days", "25",
                # "--fixed-stop-loss", "300",
            ])
        ...
        if not os.path.exists(summary_file):
            logging.warning(f"Summary file not found for {algo_name} on {interval} [{symbol}] despite success.")
            return None

        df = pd.read_csv(summary_file).fillna(0)

        # ----- Total profit (robust to Total_profit / Total_Profit) -----
        ...
        # ----- Success rate (robust to different column names / formats) -----
        success_cols = [
            "SuccessRate",
            "Success_Rate",
            "Success Rate",
        ...
        if sr_series is not None:
            sr = sr_series.astype(str).str.replace("%", "", regex=False)
            sr = pd.to_numeric(sr, errors="coerce")

            # If values look like fractions (0–1), scale to percentage (0–100)
            if sr.max() <= 1.0:
                sr = sr * 100.0
            ...
            logging.warning(
                f"No success-rate column found in summary for {algo_name} on {interval} [{symbol}]. "
...
def evaluate_one_symbol(symbol: str, trade_size: float, user_id: str, eod_close: bool) -> pd.DataFrame:
    ...
    # Save per-symbol best recommendation
...
def main():
    parser = argparse.ArgumentParser(description="AlgoMM-only Master Evaluator for StockWicks")
    ...
    if args.symbols_file:
        ...
        if not symbols:
            ...
            sys.exit(1)
        logging.info(
            f"Loaded {len(symbols)} symbols from file: "
    ...
    if combined_full_rows:
        ...
        combined_full.sort_values(by=["Symbol", "Success_Rate", "Total_Profit"],
