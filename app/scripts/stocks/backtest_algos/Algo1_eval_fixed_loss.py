...
def determine_signals(df: pd.DataFrame) -> pd.DataFrame:
    # (Keep the existing function - ATR calculation removed as it's not needed for signals here)
    df = df.copy()
...
def evaluate_performance(
    ...
    stop_loss_per_share = fixed_stop_loss_amount / qty # Calculate stop per share
...
def run(symbol: str, interval: str, trade_size: float, user_id: str, eod_close: bool, fixed_stop_loss_amount: float): # Added fixed stop
    ...
    df_cols = ['symbol', 'Interval', 'Entry_date_time', 'Entry_price', 'Exit_date_time', 'Exit_price', 'Profit', 'Status', 'Exit_Reason']
    if df.empty or interval == '1d' or len(df) < 200:
...
# --- CLI ---
def parse_args():
    p = argparse.ArgumentParser(description="Algo1: VWAP Scalper (AI Eval with Fixed $ Stop)")
