...
class EvalConfig:
    ...
    # Out-of-sample evaluation (optional)
    # - If oos_days > 0: simulate only the last N trading days (train on earlier rows)
    # - Else if test_frac > 0: simulate only the last fraction of rows (train on earlier rows)
    oos_days: int = 0
    test_frac: float = 0.0
    no_use_feat_labels: bool = False  # if True, ignore feature-builder labels
...
def _tag_model_path(model_path: str, tag: str) -> str:
    """Append a tag before the file extension (keeps the directory)."""
...
def _compute_oos_indices(
    idx: pd.DatetimeIndex,
    *,
    oos_days: int,
    test_frac: float,
    k_forward: int,
) -> Tuple[int, pd.DatetimeIndex, str]:
    """
    Returns:
      - train_end_pos: exclusive row position in idx for training slice end
      - sim_index: datetime index to simulate on (OOS slice)
      - oos_tag: '' if OOS disabled, else something like 'last_20pct' or 'last_5d'

    Leakage guard:
      We drop the last k_forward rows from the training region so training labels
      do not depend on prices inside the simulation region.
    """
    if (oos_days is None or int(oos_days) <= 0) and (test_frac is None or float(test_frac) <= 0.0):
        return len(idx), idx, ""

    # Ensure tz-aware datetime index
    dt_idx = pd.DatetimeIndex(pd.to_datetime(idx))
    if dt_idx.tz is None:
        dt_idx = dt_idx.tz_localize("UTC")

    sim_start_pos: Optional[int] = None
    sim_index: pd.DatetimeIndex

    if oos_days is not None and int(oos_days) > 0:
        oos_days = int(oos_days)
        et = dt_idx.tz_convert(ET_TZ)
        days = np.array(et.date, dtype=object)
        uniq_days = list(dict.fromkeys(days))  # stable unique preserving order

        if len(uniq_days) <= oos_days:
            raise ValueError(
                f"Not enough unique trading days for oos_days={oos_days}. Have {len(uniq_days)} days in X."
            )

        oos_set = set(uniq_days[-oos_days:])
        sim_mask = np.array([d in oos_set for d in days], dtype=bool)
        if not sim_mask.any():
            raise ValueError("OOS sim_mask empty. Check timezone/index alignment.")
        sim_start_pos = int(np.argmax(sim_mask))
        sim_index = dt_idx[sim_mask]
        oos_tag = f"last_{oos_days}d"
    else:
        test_frac = float(test_frac)
        if not (0.0 < test_frac < 1.0):
            raise ValueError("--test-frac must be between 0 and 1 (exclusive)")
        n = len(dt_idx)
        sim_n = max(1, int(round(n * test_frac)))
        sim_start_pos = n - sim_n
        sim_index = dt_idx[sim_start_pos:]
        oos_tag = f"last_{int(round(test_frac * 100))}pct"

    assert sim_start_pos is not None

    leakage_guard_drop = int(max(0, k_forward))
    train_end_pos = max(0, sim_start_pos - leakage_guard_drop)

    if train_end_pos <= 0 or len(sim_index) <= 0:
        raise ValueError(
            f"Invalid OOS split. train_end_pos={train_end_pos}, sim_rows={len(sim_index)}."
        )

    return train_end_pos, sim_index, oos_tag
...
def main(argv: Optional[List[str]] = None):
    ...
    parser.add_argument("--oos-days", type=int, default=0, help="OOS: simulate only last N trading days (train on earlier rows; drops last k_forward rows from train to prevent leakage).")
    parser.add_argument("--test-frac", type=float, default=0.0, help="OOS: simulate only last fraction of rows (e.g. 0.20). Ignored if --oos-days > 0.")
    parser.add_argument(
        "--no-use-feat-labels",
        action="store_true",
        help="If set, ignore labels created by feature builder and re-derive labels from price series (debug).",
    )
    ...
    cfg = EvalConfig(
        ...
        no_use_feat_labels=args.no_use_feat_labels,
    ...
    # Train model / predict probabilities (optionally OOS)
    feat_names = [c for c in feat_df.columns if c not in ("y", "w")]
    if cfg.no_use_feat_labels:
        feat_names = _DEFAULT_FEATS
    # Models output directory
    models_dir = MODEL_DIR
    os.makedirs(models_dir, exist_ok=True)


    model_path = os.path.join(models_dir, f"mm2_{cfg.symbol}_{cfg.interval}_k{cfg.k_forward}.joblib")

    # Prepare X for all rows (drops NaNs)
    X_all = _prepare_X(feat_df, feat_names)

    # ----------------- Out-of-sample (OOS) split (optional) -----------------
    train_end_pos, sim_index, oos_tag = _compute_oos_indices(
        X_all.index,
    ...
    price_df_sim = price_df
    X_pred = X_all
    if oos_tag:
        # Tag outputs & model so OOS runs don't overwrite in-sample outputs
        prefix = f"{prefix}_OOS_{oos_tag}"
        model_path = _tag_model_path(model_path, f"OOS_{oos_tag}")

        X_train = X_all.iloc[:train_end_pos]
        train_feat_df = feat_df.loc[X_train.index].copy()

        logger.info(
            f"OOS enabled ({oos_tag}): train_rows={len(X_train)} (end={X_train.index.max()}), "
            f"sim_rows={len(sim_index)} (start={sim_index.min()}, end={sim_index.max()}), "
            f"leakage_guard_drop={cfg.k_forward}"
        )

        # Train only on TRAIN slice (always train for OOS so results are consistent)
        ...
        X_pred = X_all.loc[sim_index]
        if len(X_pred) == 0:
            raise RuntimeError("OOS split produced empty prediction slice (X_pred).")

        # Slice price_df to simulation window
        sim_start_ts = X_pred.index.min()
        price_df_sim = price_df.loc[price_df.index >= sim_start_ts]
    else:
        # In-sample (original behavior)
        model, used_feats = _load_or_train_model(
            model_path,
            feat_df,
            feat_names,
    ...
    probs = predict_probability(model, X_pred, used_feats)
    if probs is None or len(probs) == 0:
        raise RuntimeError("predict_probability returned no probabilities.")

    # Raw model probability for the *current bar* (index-aligned to X_pred)
    prob_raw = pd.Series(probs, index=X_pred.index, name="prob_up_raw")

    # Optional smoothing: trailing (past-only) rolling mean over k_forward bars
    ...
    trades_long, trades_short, run_df = _simulate_trades(cfg, price_df_sim, prob_series)
