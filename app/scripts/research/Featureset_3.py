#!/usr/bin/env python3
"""
StockWicks Algo3_MM SMI-only feature-set builder.

Commercial version:
- Featureset_3: SMI-only features for Algo3_MM.
- Live-safe inference builder keeps the latest candle.
- Training builder adds forward labels and drops only unlabeled rows.

Install target:
  /var/stockwicks/clients/ashakil/app/scripts/research/Featureset_3.py
"""
...
logger = logging.getLogger("Algo3_MM_FeatureSet")
if not logger.handlers:
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(levelname)s (Algo3_MM_FeatureSet) %(message)s",
    )

from app.utils.stock.indicators import compute_smi_blau  # noqa: E402
...
FEATURESETS = {
    "Featureset_3": FEATURESET_3,
    # Backwards-compatible aliases:
    "FeatureSet_3": FEATURESET_3,
    "featureset_3": FEATURESET_3,
    "algomm_v3": FEATURESET_3,
}


def get_feature_columns(feature_set: str = "Featureset_3") -> list[str]:
    """Return the selected model feature column names."""
    name = str(feature_set or "Featureset_3").strip()
    return list(FEATURESETS.get(name, FEATURESET_3))
...
def _smi_features(df: pd.DataFrame) -> pd.DataFrame:
    smi_k, smi_d = compute_smi_blau(
        df,
        length=10,
        r=3,
        s=3,
    ...
    smi_k = smi_k.reindex(df.index).replace([np.inf, -np.inf], np.nan)
    smi_d = smi_d.reindex(df.index).replace([np.inf, -np.inf], np.nan)
    smi_hist = (smi_k - smi_d).replace([np.inf, -np.inf], np.nan)
    smi_slope = smi_k.diff().replace([np.inf, -np.inf], np.nan)

    return pd.DataFrame(
        {
            "smi_k": (smi_k / 100.0).clip(-2.0, 2.0),
            "smi_d": (smi_d / 100.0).clip(-2.0, 2.0),
            "smi_hist": (smi_hist / 100.0).clip(-2.0, 2.0),
            "smi_slope": (smi_slope / 100.0).clip(-2.0, 2.0),
        },
        index=df.index,
