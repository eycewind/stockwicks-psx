...
FIX (2026-01-26):
- Enforce max_allocation_usd sizing cap (qty <= floor(max_alloc / (entry_price*100)))
- Respect forced contracts (bot_params["contracts"]) as the highest priority override
- Adds sizing debug fields in candidate plans + best_overall_trade
"""
import os
...
def _cap_contracts_by_allocation(
    ...
    """
    Caps contracts so estimated allocation (entry_price * 100 * qty) <= max_allocation_usd.

    Returns: (final_qty, alloc_cap_qty, est_alloc_used)
      - alloc_cap_qty is the cap derived from allocation (or None if not applicable)
      - est_alloc_used is based on final_qty (or None if entry_price invalid)
    """
    if desired_contracts is None or desired_contracts < 1:
        desired_contracts = 1

    if entry_price is None or entry_price <= 0:
        return int(max(1, desired_contracts)), None, None

    if max_allocation_usd is None or max_allocation_usd <= 0:
        est_alloc = round(entry_price * 100.0 * float(desired_contracts), 2)
        return int(max(1, desired_contracts)), None, est_alloc

    alloc_cap_qty = int(max_allocation_usd // (entry_price * 100.0))
    alloc_cap_qty = max(1, alloc_cap_qty)
    final_qty = int(min(desired_contracts, alloc_cap_qty))
...
async def run_guru_pick_4exp_bots(bot_params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Running guru_pick_4exp bot for {symbol} with strategy={strategy}, style={style}, "
            f"forced_contracts={forced_contracts}, max_allocation_usd={max_allocation_usd}"
        )
        ...
        for exp_date in scan_dates:
            ...
            for _, row in top_candidates.iterrows():
                ...
                # --- SIZING ENFORCEMENT (ORDER MATTERS) ---
                # base desired qty from stop model
                base_qty = int(row.get("position_size", 1)) if row.get("position_size") is not None else 1

                # forced contracts overrides everything
                desired_qty = int(forced_contracts) if forced_contracts is not None else base_qty

                final_qty, alloc_cap_qty, est_alloc_used = _cap_contracts_by_allocation(
                    entry_price=float(row["entry_price"]),
                    desired_contracts=desired_qty,
                    max_allocation_usd=max_allocation_usd,
                )

                # Mutate row so downstream uses correct qty
                row["position_size"] = int(final_qty)

                # Debug sizing fields
                plan["position_size_base"] = int(base_qty)
                plan["position_size_forced"] = int(forced_contracts) if forced_contracts is not None else None
                plan["position_size_alloc_cap"] = int(alloc_cap_qty) if alloc_cap_qty is not None else None
                plan["position_size_final"] = int(final_qty)
                plan["estimated_allocation_used"] = float(est_alloc_used) if est_alloc_used is not None else None
                plan["max_allocation_usd"] = float(max_allocation_usd) if max_allocation_usd is not None else None
        ...
        if best_overall and best_overall_exp is not None:
            entry = float(best_overall["entry_price"])
            qty = int(best_overall.get("position_size", 1) or 1)
            est_alloc = round(entry * 100.0 * qty, 2)

            response["best_overall_trade"] = {
                "expiration": best_overall_exp.strftime("%Y-%m-%d"),
                # Option OCC symbol (important for trade creation)
                "symbol": str(best_overall.get("symbol") or ""),
                ...
                # sizing debug
                "estimated_allocation_used": est_alloc,
                "max_allocation_usd": float(max_allocation_usd) if max_allocation_usd is not None else None,
                "position_size_base": best_overall.get("position_size_base"),
                "position_size_forced": best_overall.get("position_size_forced"),
                "position_size_alloc_cap": best_overall.get("position_size_alloc_cap"),
                "position_size_final": best_overall.get("position_size_final"),
            }
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--max-allocation-usd", type=float, default=None,
                   help="Hard cap on estimated allocation (entry_price*100*qty). Overrides env GURU_MAX_ALLOCATION_USD when provided.")
...
def main() -> int:
    ...
    bot_params = {
        ...
        "max_allocation_usd": float(args.max_allocation_usd) if args.max_allocation_usd is not None else None,
    }
    ...
    if "best_overall_trade" in result:
        ...
        if best.get("max_allocation_usd") is not None:
            print(f"\n💰 SIZING (Allocation Cap):")
            print(f"Max Allocation:   ${best['max_allocation_usd']:.2f}")
            print(f"Est Allocation:   ${best['estimated_allocation_used']:.2f}")
            print(f"Base/Forced/Cap:  {best.get('position_size_base')} / {best.get('position_size_forced')} / {best.get('position_size_alloc_cap')}")
            print(f"Final Qty:        {best.get('position_size_final')}")
    ...
    for exp_date, exp_data in result["results_by_expiration"].items():
        ...
        print(f"   {'Rank':<4} {'Action':<12} {'Strike':<8} {'Entry':<7} {'Δ':<5} {'IV%':<5} {'Score':<6} {'Conf':<6} {'Pos':<4} {'Alloc$':<8}")
        print(f"   {'-'*80}")

        for i, candidate in enumerate(exp_data["candidates"][:5], 1):  # Show top 5
            entry = candidate['entry_price']
            action = candidate['action']
            alloc_used = candidate.get("estimated_allocation_used")
            alloc_str = f"{alloc_used:.0f}" if isinstance(alloc_used, (int, float)) and alloc_used is not None else "-"
            print(
                ...
                f"{candidate['position_size']:<4} {alloc_str:<8}"
            )
